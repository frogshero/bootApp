...
public class ClientCnxn {
  ...
  /**
   * @@apiNote zxid Zookeeper服务器上当前最新的事务ID
   */
   ...
   */
    ...
    public Packet queuePacket(RequestHeader h, ReplyHeader r, Record request,
      ...
      /**
       * @@apiNote Selector.select()是一个阻塞方法，只在至少一个channel被选中或wakeup方法被调用，或线程中断才返回。
       * Selector.select()的注释：Causes the first selection operation that has not yet returned to return immediately.
       * If another thread is currently blocked in an invocation of the select() then that invocation
       * will return immediately.  If no selection operation is currently in
       * progress then the next invocation of one of these methods will return
       * immediately unless the selectNow method is invoked in the meantime.
       * In any case the value returned by that invocation may be non-zero.
       * Subsequent invocations of the select methods will block as usual unless this method is invoked
